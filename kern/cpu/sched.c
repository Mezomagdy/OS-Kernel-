...
struct Env* fos_scheduler_PRIRR()
{
	...
	struct Env* cur=get_cpu_proc();
	    ...
	    for (int p = 0; p < num_of_ready_queues; p++)
	    {
	        ...
	        {
	            next = dequeue(&ProcessQueues.env_ready_queues[p]);
	            break;
	        }
	    }
...
void clock_interrupt_handler(struct Trapframe* tf)
{
  ...
  if (isSchedMethodPRIRR())
  {
    ...
    LIST_FOREACH(proc,&(ProcessQueues.env_ready_queues[i]))
    {
      ...
      if (proc->prrticks >= starvationThreshold && proc->priority > 0)
      {
        proc->need_promotion = 1;
      }
    }
    LIST_FOREACH(proc,&(ProcessQueues.env_ready_queues[i]))
    {      if (proc->need_promotion)
        {
          proc->need_promotion = 0;
          proc->prrticks = 0;
          proc->priority--;
...
void update_WS_time_stamps()
{
	 struct Env* curn = get_cpu_proc();
	    ...
	    struct WorkingSetElement *el2;
	    LIST_FOREACH(el2, &(curn->page_WS_list))
	    {
	        uint32 va = el2->virtual_address;
	        uint32 *pt = NULL;

	        get_page_table(curn->env_page_directory, va, &pt);
	        ...
	        if(pt != NULL)
	        {
	            ...
	            if(pte & PERM_USED)
	            {
	            	el2->time_stamp |= 0x80000000;
	            }


	            pt_set_page_permissions(curn->env_page_directory, va, 0, PERM_USED);
	        }
	    }
