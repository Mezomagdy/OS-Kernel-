...
{
	int khm=  0xFFFFF000 ;
	int kstSIZE= (8*PAGE_SIZE)  ;
#if !USE_KHEAP
	{
		if (khm - __cur_k_stk < kstSIZE)
...
#else
	void* stck = kmalloc(kstSIZE);
	    if(stck==NULL){
	        panic("failed to create");
	    }
	     uint32* gp = (uint32*)stck;
	     uint32* ptrt = NULL;
	     get_page_table (ptr_user_page_directory, (uint32)gp,&ptrt);
	     if (ptrt != NULL){
	         ptrt[PTX(gp)]&= ~PERM_PRESENT;
	     }
	     else{
	         panic("failed to set guard");
	     }
	return (void*)(uint32)stck;
